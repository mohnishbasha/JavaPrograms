package com.algorithms.palindrome.subsequence.longestpalindrome.manachers;
...
s2 be a derived string of s, comprising N * 2 + 1 elements, with each data corresponding to one of the following:
the N characters in s, the N-1 boundaries among characters, and the boundaries before and after the first and last
character respectively

A boundary in s2 is equal to any other boundary in s2 with respect to data matching in palindromic length
determination

p be an array of palindromic span for each data in s2, from center to either outermost data, where each boundary
...
i be the position of an data (i.e., a character or boundary) in s2 whose palindromic span is being determined, with
...
- Vivekanand
https://www.youtube.com/watch?v=yZWmS6CXbQc&t=64s
