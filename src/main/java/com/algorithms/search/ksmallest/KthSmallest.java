...
In this post method 4 is discussed which is mainly an extension of method 3 (QuickSelect) discussed in the previous post. The idea is to randomly pick a pivot data. To implement randomized partition, we use a random function, rand() to generate index between l and r, swap the data at randomly generated index with the last data, and finally call the standard partition process which uses last data as pivot.

Following is implementation of above Randomized QuickSelect.

 */

// Java program to find k'th smallest data in expected
// linear time
class KthSmallest
{
